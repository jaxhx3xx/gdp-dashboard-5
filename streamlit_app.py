...
def load_data():
    ...
    return gmsl_df, factors_df, map_data, nutrition_df, plastic_df, debris_df, fish_production_df, aquaculture_df, ghg_df
...
st.markdown("지구 온난화로 인한 해수면 상승은 단순히 해안선이 변하는 문제를 넘어, 해양 생태계를 교란하고 결국 우리의 식탁까지 위협하는 연쇄적인 위기입니다. 이 대시보드는 그 거대한 위협의 흐름을 데이터로 분석하고 미래를 조망합니다.")
st.markdown("---")
...
with tab1:
    ...
    map_selected_year = st.slider(
        '지도 데이터 연도 선택',
        min_value=int(map_df['연도'].min()),
        max_value=int(map_df['연도'].max()),
        value=int(map_df['연도'].max())
    )
    map_filtered = map_df[map_df['연도'] == map_selected_year]
    
    fig_map = px.choropleth(map_filtered, locations="country_iso", color="rise_rate_mm_year", 
                            hover_name="country_iso", 
                            color_continuous_scale=px.colors.sequential.Blues, 
                            title=f"{map_selected_year}년 국가별 연평균 해수면 상승률 (mm/년)",
                            range_color=(2, 8)) # 색상 범위를 고정하여 연도별 비교가 용이하게 함
    ...
    ghg_year_range = st.slider(
        '온실가스 데이터 분석 기간 선택',
        min_value=int(ghg_df['연도'].min()),
        max_value=int(ghg_df['연도'].max()),
        value=(int(ghg_df['연도'].min()), int(ghg_df['연도'].max()))
    )
